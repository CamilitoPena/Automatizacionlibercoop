...
(async () => {
  ...
  await (await client.$('id=com.libercoop.appliber:id/nav_contacto')).click();
  const indicatorText = await client.$('id=com.libercoop.appliber:id/txtMensajeSuperior');
  const isVisible = await indicatorText.isDisplayed().catch(() => false);

  console.log('Resultado acceso a Contacto:', isVisible ? '✅ Correcto' : '❌ Falló');
