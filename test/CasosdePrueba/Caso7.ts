...
(async () => {
  ...
  await (await client.$('id=com.libercoop.appliber:id/nav_sucursales')).click();
  const indicatorText = await client.$('id=com.libercoop.appliber:id/txtNombreCasaMatriz');
  const isVisible = await indicatorText.isDisplayed().catch(() => false);

  console.log('Resultado acceso a Sucursales:', isVisible ? '✅ Correcto' : '❌ Falló');
