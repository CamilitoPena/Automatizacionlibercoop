...
(async () => {
  ...
  const VisualCRE = await client.$('id=com.libercoop.appliber:id/txtCrditos');
  await VisualCRE.waitForDisplayed({ timeout: 10000 });
  console.log('✅ Interfaz de créditos visible:', await VisualCRE.getText());

  const VisualLDA = await client.$('id=com.libercoop.appliber:id/txtLibretadeAhor');
  await VisualLDA.waitForDisplayed({ timeout: 10000 });
  console.log('✅ Interfaz de Libretas de ahorro visible:', await VisualLDA.getText());

  const VisualDAP = await client.$('id=com.libercoop.appliber:id/txtDepsitosaPla');
  await VisualDAP.waitForDisplayed({ timeout: 10000 });
  console.log('✅ Interfaz de Depósitos a plazo visible:', await VisualDAP.getText());
