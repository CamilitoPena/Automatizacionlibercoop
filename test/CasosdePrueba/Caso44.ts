...
(async () => {
 ...
 await client.pause(2000);
  ...
  const btnListaDAP = await client.$('.//androidx.recyclerview.widget.RecyclerView[@resource-id="com.libercoop.appliber:id/recyclerview"]/android.view.ViewGroup');
  await btnListaDAP.waitForDisplayed({ timeout: 10000 });
  await btnListaDAP.click();
  await client.pause(1000);

  try {
    const totalInversion = await client.$('id=com.libercoop.appliber:id/textviewNombreDepo');
    console.log('✅ Información de DAP visible:', await totalInversion.isDisplayed());
  } catch {
    console.log('❌ No se encontró información del DAP');
  }
